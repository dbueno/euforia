...
void mylog(const vector<z3::expr>& e) { std::cerr << e << std::endl; }
void mylog(const z3::expr_vector& v) { std::cerr << v << std::endl; }
void mylog(const euforia::Cube& c) { std::cerr << c << std::endl; }
void mylog(const euforia::TimedCube& t) { std :: cerr << t << std::endl; }
