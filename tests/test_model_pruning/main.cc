...
// Set to true and this test terminates in a few seconds.
bool fast = false;
...
class ModelPruningTraversalT {
 public:
  ...
  struct StackElt {
    z3::expr node;
    // Either not set (so bool(node_eval) is false) or set to what Eval(node)
    // is
    z3::expr node_eval;
    std::vector<std::pair<z3::expr, z3::expr>> args;
    size_t arg_index;
  ...
  class Iterator : public IteratorStorage<SetType> {
   ...
   private:
    ...
    void PruneArgs(StackElt *elt) const {
      // Adds a arg to visit
      auto push_arg = [&](const z3::expr& e, const z3::expr& e_eval) {
        elt->args.push_back(std::make_pair(e, e_eval));
      };
      ...
      switch (kind) {
        case Z3_OP_ITE: {
          ...
          assert(expr_is_true(cond_eval) || expr_is_false(cond_eval));
          ...
          push_arg(cond, cond_eval);
          push_arg(true_branch, z3::expr(node.ctx()));
          break;
        }
        ...
        case Z3_OP_OR: {
          ...
          assert(expr_is_true(node_eval) || expr_is_false(node_eval));
          ...
          // Caches the arg's evaluation so it can be reused for the
          // next round.
          elt->args.reserve(node.num_args());
          for (unsigned i = 0; i < node.num_args(); i++) {
            if (fast) {
              // Pushes arg onto stack and sets its eval to node eval
              push_arg(node.arg(i), node_eval);
            } else {
              // Pushes arg onto stack and sets its eval to unknown
              push_arg(node.arg(i), z3::expr(node.ctx()));
            }
          }
          break;
        }
        ...
        default:
          elt->args.reserve(node.num_args());
          // Traverses all the args by default.
          for (unsigned i = 0; i < node.num_args(); i++) {
            push_arg(node.arg(i), z3::expr(node.ctx()));
          }
          break;
...
int main(int argc, char **argv) {
  ...
  s.push();
  auto p = ctx.constant("boolP!0", ctx.bool_sort());
  s.add(p);
  auto r = s.check();
  if (r == sat) {
    auto m = s.get_model();
    auto eval = [&](const z3::expr& e, bool c) { return m.eval(e, c); };
    ModelPruningTraversalT t(eval);
    std::unordered_set<z3::expr, HashExpr, EqualToExpr> visited;
    int num_evals = 0;
    for (unsigned i = 0; i < assertions.size(); i++) {
      auto assertion = assertions[i];
      // cout << "assertion" << endl;
      // cout << assertion << endl;
      for (auto it = t.begin(assertion, visited),
           ie = t.end(assertion, visited); it != ie; ++it) {
    ...
    cout << "did " << num_evals+t.num_evals() << " evals\n";

    // vector<z3::expr> assumps;
    // assumps.push_back(p);
    // auto smt2_string = ToSmt2WithAssumps(assertions, assumps);
    // stringstream str_out;
    // str_out << smt2_string;
    // for (auto& e : t.eval_exprs()) {
    //   str_out << "(eval " << e << ")\n";
    // }
    // cout << str_out.str();
  }
